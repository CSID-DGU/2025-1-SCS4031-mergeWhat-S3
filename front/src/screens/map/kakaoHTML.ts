    ...
    if (locations && Array.isArray(locations)) {
        locations.forEach(locationData => {
            ...
            if (locationData.lat !== null && locationData.lat !== undefined && locationData.lng !== null && locationData.lng !== undefined) {
                ...
                console.warn("경고: 유효하지 않은 가게 위치 데이터 (위도 또는 경도 없음):", locationData);
                alert("경고: 유효하지 않은 가게 위치 데이터 (위도 또는 경도 없음):" + JSON.stringify(locationData)); // 디버깅용 alert
            }
        });
        ...
        console.warn("경고: locations가 유효한 배열이 아니거나 비어있습니다.", locations);
...
function addSingleMarker(locationData) {
    ...
    if (locationData.lat !== null && locationData.lat !== undefined && locationData.lng !== null && locationData.lng !== undefined) {
        ...
        console.warn("경고: 유효하지 않은 단일 가게 위치 데이터 (위도 또는 경도 없음):", locationData);
...
if (window.ReactNativeWebView) {
  document.addEventListener('message', function(event) {
    ...
    alert(' setParkingMarkers 수신'); // 디버깅용
      ...
      alert("웹뷰 메시지 처리 오류: " + e.message + ", 원본 데이터: " + event.data);
    }
  });
} else {
  ...
  alert("경고: window.ReactNativeWebView가 정의되지 않았습니다.");
        ...
        // 주차장 관련 전역 변수 (기존 'markers', 'infowindow'와는 별개)
        ...
        function hideMarketElements() {
            ...
            allIndoorPolygons.forEach(({ polygons }) => polygons.forEach(p => p.setMap(null))); // 모든 실내 폴리곤 숨기기
