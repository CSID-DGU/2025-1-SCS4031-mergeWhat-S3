  ...
  <body>
    ...
    <script>
      ...
      kakao.maps.event.addListener(map, "zoom_changed", function () {
        ...
        allMainPolygons.forEach((polygon) => polygon.setMap(showIndoor ? null : map));
      ...
      let markerList = [];  // 기존 마커들을 지우기 위해 필요

document.addEventListener("message", function (event) {
  alert("카테고리 웹뷰로부터 수신한 메시지:", event.data);
  const message = JSON.parse(event.data);

  if (message.type === "showMarkers" && Array.isArray(message.markers)) {
    alert("마커 생성 요청 수신됨: ",mesage.markers)
    // 기존 마커 제거
    markerList.forEach(marker => marker.setMap(null));
    markerList = [];

    try{
    message.markers.forEach(({ lat, lng }) => {
