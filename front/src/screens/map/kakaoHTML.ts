...
<html lang="ko">
  <head>
    ...
    <style>
      ...
      /* 주차장 마커용 인포윈도우 스타일 (기존 kakaoHTML에 없었지만 추가) */
      .parking-infowindow {
        padding: 5px;
        font-size: 12px;
      ...
      /* 기존 시장 폴리곤 오버레이 스타일 (이전 답변에서 임의 추가했던 스타일) */
      .info-bubble {
        background-color: white;
        border: 1px solid #ccc;
        border-radius: 5px;
        ...
        white-space: nowrap;
        text-align: center;
        pointer-events: none; /* 오버레이 아래 지도를 클릭할 수 있도록 */
      }
    </style>
  </head>
  <body>
    ...
    <script>
      ...
      // 마커 띄우기
      var markers = []; // 지도에 표시될 마커들을 담을 배열
      ...
      // 🚩 여러 마커를 추가하는 함수 (MapHomeScreen.tsx의 'addMultipleMarkers'와 매칭)
      function addMultipleMarkers(locations) {
        removeAllMarkers(); // 새로운 마커를 그리기 전에 기존 마커 제거
        ...
        if (locations && Array.isArray(locations)) {
          locations.forEach(locationData => {
            if (
              ...
              var marker = new kakao.maps.Marker({
                map: map, // 전역 'map' 객체 사용
                position: markerPosition,
                // 여기에 마커 이미지 설정 등 추가 가능 (예: image 속성 사용)
              });

              markers.push(marker);

              // 마커 정보창 (InfoWindow)
              var storeName = locationData.name || '이름 없음';
              ...
              // 마커 클릭 시 정보창 열기 이벤트 리스너
              kakao.maps.event.addListener(marker, 'click', function () {
                infowindow.open(map, marker);
              });
            } else {
              console.warn(
                '경고: 유효하지 않은 가게 위치 데이터 (위도 또는 경도 없음):',
                locationData,
              );
              alert(
                '경고: 유효하지 않은 가게 위치 데이터 (위도 또는 경도 없음):' +
          ...
          console.warn(
            '경고: locations가 유효한 배열이 아니거나 비어있습니다.',
            locations,
          );
        }
      }

      // 🚩 단일 마커를 추가하는 함수 (MapHomeScreen.tsx의 'addSingleMarker'와 매칭)
      function addSingleMarker(locationData) {
        ...
        if (
          ...
          var storeName = locationData.name || '이름 없음';
          var infowindowContent =
            '<div style="padding:5px;font-size:12px;">' + storeName + '</div>';
          ...
          console.warn(
            '경고: 유효하지 않은 단일 가게 위치 데이터 (위도 또는 경도 없음):',
            locationData,
          );
        }
      }
      ...
      if (window.ReactNativeWebView) {
          ...
          try {
            ...
            switch (message.type) {
              ...
              case 'SET_MAP_MODE':
                ...
                //alert( "MAP mode 설정됨: " + window.currentMapMode + "");
              ...
              case 'setParkingMarkers':
                if (window.currentMapMode === 'parking') {
                 alert('📡 강제로 수신 테스트');
                  alert(' setParkingMarkers 수신');

                  removeAllParkingMarkers();
                  currentParkingPlaces = message.payload;
                  message.payload.forEach(function (p) {
                    addParkingPlaceMarker(p); // 이 함수가 정의돼 있어야 함
                  });

                  alert(' setParkingMarkers 수신'); // 디버깅용
                }
                break;

              case 'MOVE_TO_PARKING_PLACE':
                if (
                  window.currentMapMode === 'parking' &&
                  message.lat &&
                  message.lng
                ) {
                  ...
                  const targetPlace = currentParkingPlaces.find(
                    p =>
                      parseFloat(p.LAT) === message.lat &&
                      parseFloat(p.LNG) === message.lng,
                  );
                  ...
                  if (targetPlace) {
                    for (let i = 0; i < parkingDataMarkers.length; i++) {
                      ...
                      if (
                        pos.getLat() === message.lat &&
                        pos.getLng() === message.lng
                      ) {
                        const content =
                          '<div class="parking-infowindow">' +
                          targetPlace.PARKING_NAME +
                          '<br>' +
                          targetPlace.ADDR +
                          '</div>';
            ...
            console.error('웹뷰 메시지 처리 중 오류 발생:', e, event.data);
            /*alert(
              '웹뷰 메시지 처리 오류: ' +
                e.message +
                ', 원본 데이터: ' +
        ...
        console.warn('window.ReactNativeWebView가 정의되지 않았습니다.');
        alert('경고: window.ReactNativeWebView가 정의되지 않았습니다.');
      }

      // ===============================================
      // parkingHTML.ts 기능에 필요한 추가 전역 변수 및 함수 (기존 코드 아래에 추가)
      // ===============================================

      // 웹뷰의 현재 지도 모드를 추적하는 변수
      // 'market' (기본값): 시장 폴리곤 및 상점 마커 표시
      // 'parking': 주차장 마커 표시
      window.currentMapMode = 'market';

      // 시장 관련 요소들을 숨기는 함수
      function hideMarketElements() {
        allMainPolygons.forEach(polygon => polygon.setMap(null)); // 모든 메인 폴리곤 숨기기
        allIndoorPolygons.forEach(({polygons}) =>
          polygons.forEach(p => p.setMap(null)),
        ); // 모든 실내 폴리곤 숨기기
        removeAllMarkers(); // 기존 상점 마커 제거 (기존 removeAllMarkers 함수 재사용)
        infowindow.close(); // 기존 상점 인포윈도우 닫기
        // 사용자님의 기존 코드에는 customOverlay 변수가 없으므로, 이 부분을 건드리지 않습니다.
        // mainPolygons, allIndoorPolygons 변수는 이미 정의되어 있으므로 그대로 사용합니다.
      }

      // 주차장 관련 요소들을 숨기고 시장 관련 요소들을 다시 표시할 준비를 하는 함수
      function showMarketElements() {
        removeAllParkingMarkers(); // 모든 주차장 마커 제거
        parkingInfoWindow.close(); // 주차장 인포윈도우 닫기
      }

      // 주차장 마커 생성 및 정보 창 표시 함수
      function addParkingPlaceMarker(place) {
        ...
        const imageSrc =
          'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/marker_number_orange.png';
        ...
        alert('✅ 마커 추가: ' + place.PARKING_NAME);
      ...
      // 지도 이동 또는 확대/축소 시 주차장 데이터 요청 (idle 이벤트)
      // 이 리스너는 map 객체가 생성된 후에 한 번만 추가됩니다.
